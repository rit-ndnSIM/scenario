...
def run_category_task(run_id, pair, generated_workflows):
    ...
    # filename is accurate) and each run gets its own DAGs. Previously all sizes were handed to every
    # scenario at once, and all runs shared one set of workflows.
            ...
            for edgeratio in EDGERATIO_LIST:
                ...
                for ccr in CCR_LIST:
                    ...
                    for hostRatio in HOSTRATIO_LIST:
                        ...
                        # num_services must be part of the hosting filename now that it is swept: the
                        # hosting file maps services onto routers, so a 2-service and an 8-service run
                        # produce different content. Without it they collide on one name and each pass
                        # silently overwrites the last (harmless today because build_scenario embeds the
                        # content immediately, but the leftover file on disk would be ambiguous).
...
if __name__ == "__main__":
    ...
    print(f"Pre-generating {POISSON_NUM_CONSUMERS} unique workflows per category, per service count, per run, for {NUM_RUNS} runs...")
    # Keyed by (run_id, category, num_services).
    # num_services is in the key rather than category alone: keying by category only made every service
    # count land in the same list, so a single scenario ended up with one consumer per service count
    # (dag1=2srv, dag2=4srv, ...) all running at once - which is not a scalability sweep, and left no
    # single "number of services" to name the scenario after.
    # run_id is in the key so each of the NUM_RUNS runs is an independent sample of workflow structure
    # as well as of topology and hosting. Generating once and handing the same DAGs to every run baked
    # any quirk of that single set into all NUM_RUNS samples, understating the run-to-run spread.
    generated_wfs = {}

    # Extract unique workflow categories from pairs to avoid duplicate generation.
    # Categories appearing in more than one pair still share one workflow set within a run, so those
    # pairs stay a controlled topology-only comparison.
