            ...
            for ccr in CCR_LIST:
                ...
                for hostRatio in HOSTRATIO_LIST:
                    ...
                    for prefix in PREFIX_LIST:
                        ...
                        build_scenario(f"{base_name}--1-noSD2-multicast.json", wf_filenames, tp, hs, prefix, "multicast", 0, sim_end_time, sd=0, ru=0)
                    
                        # Scenario 2: noSD2, bestRoute, cs=0
                        build_scenario(f"{base_name}--2-noSD2-bestRoute.json", wf_filenames, tp, hs, prefix, "best-route", 0, sim_end_time, sd=0, ru=0)
                    
                        # Scenario 3: SD2, bestRoute, noUtil, cs=0
                        build_scenario(f"{base_name}--3-SD2-noUtilization.json", wf_filenames, tp, hs, prefix, "best-route", 0, sim_end_time, sd=2, ru=0)
                    
                        # Scenario 4: SD2, bestRoute, Util, noCaching, cs=0
                        build_scenario(f"{base_name}--4-SD2-utilization-noCaching.json", wf_filenames, tp, hs, prefix, "best-route", 0, sim_end_time, sd=2, ru=1)
