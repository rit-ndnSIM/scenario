...
workdir="$PWD/generated_scenarios/lat_bw_cpm"
mkdir -p "$workdir"
...
num_services_list="5 10 15 20 25"
num_nodes_list="4 8 12 16 20"
    ...
    for wf_topo_pair in $wf_topo_pairs; do
        ...
        for tp in $generated_topos; do
            for wf in $generated_wfs; do
                ...
                prefixes="nesco nescoSCOPT orchA orchB"
                for prefix in $prefixes; do
                    ...
                    cp "${output_filename}" ../cascon_lat-bw-cpm/
                done
                ...
                if [ "$VISUALIZE" = true ]; then
                    ...
                    if [ "$current_nodes" -lt 50 ] && [ "$current_servs" -lt 50 ]; then
